"""
Contains the Merchant class which represents a merchant in the SQL database.
"""
...
from src.model import database, transaction, location, tag
...
class Merchant(SqlObject):
    ...
    def sync(self) -> None:
        """
        Syncs this merchant to the database by updating the database. If the merchant is not in
    ...
    def get_all() -> list[Merchant]:
        ...
        return [Merchant(*data) for data in cur.fetchall()]
    ...
    def add_default_tag(self, new_tag: tag.Tag) -> None:
        """
        Adds a default tag.

        :param new_tag: Tag to be added
        ...
        try:
            cur.execute(
                ...
                (self.sqlid, new_tag.sqlid),
            )
        except IntegrityError as error:
            raise ValueError(
                f"Cannot add duplicate default tag '{new_tag.name}'."
            ) from error
    ...
    def remove_default_tag(self, tag_id: int) -> None:
        ...
        if cur.fetchone() is None:
            raise KeyError(
                f"Merchant '{self.name}' does not have a default tag '"
                f"{tag.Tag.from_id(tag_id).name}'."
            )
